...
var guioffset = 128 #-: gui space on top, +: gui space on bottom
var offsetit = [128,64]
...
#		size.x = view_size.x
	limit_top = collision_shape.position.y+area.position.y - size.y
...
func zoomer(area): #if (area.get_parent().scale.x < 1 || area.get_parent().scale.y < 1)
